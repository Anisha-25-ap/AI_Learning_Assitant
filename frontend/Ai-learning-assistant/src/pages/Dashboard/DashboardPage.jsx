...
const DashboardPage = () => {
  ...
  useEffect(() => {
    const fetchDachboardData = async () => {
      try {
        // ERROR YAHAN THI: progressService. hatana hoga
        const data = await getDashboardData(); 
        console.log("Data___getDashboardData", data);
        const finalData = data.data ? data.data : data;

        // Aksar axios response mein data.data hota hai, check karein aapka API kya bhej raha hai
  ...
  const stats = [
    ...
    },
    {
      ...
      value: DashboardData.overview.totalFlashcards, // Yahan totalDocuments likha tha, maine sahi kar diya
      icon: BookOpen,
    ...
    <div className="min-h-screen">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-5">
        {stats.map((stat, index) => (
          <div
            key={index}
            className="group relative bg-white/80 backdrop-blur-xl border border-slate-200/60 rounded-2xl shadow-xl shadow-slate-200/50 p-6 hover:shadow-2xl hover:shadow-slate-300/50 transition-all duration-300 hover:-translate-y"
          >
      ...
      {/*Recent Action */}
        <div className="bg-white/80 backdrop-blur-xl border border-slate-200/60 rounded-2xl shadow-slate-200/50 p-8">
          <div className="flex items-center gap-3 mb-6">
            <div className="w-10 h-10 rounded-xl bg-linear-to-br from-slate-100 to-slate-200 flex items-center justify">
                    ...
                    >
                      <div className="flex-1 min-w-0">
                        ...
                        {activity.link && (
                          <a
                            href={activity.link}
                            className="ml-4 px-4 py-2 text-xs font-semibold text-emerald-600 hover:text-emerald-700 hover:bg-emerald-50 rounded-lg transition-all duration-200 whitespace-nowrape"
                          >
                            View
                          </a>
                        )}
